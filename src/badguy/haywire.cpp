...
Haywire::Haywire(const ReaderMapping& reader) :
  ...
  is_stunned(false),
  time_stunned(0.0f),
  m_lightsprite(SpriteManager::current()->create("images/creatures/haywire/ticking_glow/ticking_glow.sprite")),
  m_jumping(false),
  m_skid_timer(),
  ...
  if (is_exploding)
  {
    ...
    //end of pathfinding
      ...
      if (m_jumping)
      {
        ...
        m_lightsprite->set_action("jump", /* loops = */ 1);
      }
